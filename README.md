...
The changes widget remains display-only. Git Explorer supports shell-free per-file stage/unstage and guarded tracked-file discard with automatic refresh. Patch-level staging, untracked deletion, conflict discard, commit composition, and replacement of Pi's transcript renderer remain deferred. Embedded Vim mode intentionally implements only the documented core motions; it is not a Vim emulator.
